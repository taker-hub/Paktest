...
uninstall_nahan() {
    ...
    # Execute actual removal commands (ignoring failures gracefully)
    run_with_spinner " ${INFO} Removing D1 Database '$DB_NAME' from Cloudflare Edge..." npx wrangler d1 delete "$DB_NAME" --force
    run_with_spinner " ${INFO} Deleting Edge Worker script '$WORKER_NAME'..." npx wrangler delete --name "$WORKER_NAME"

    # Remove local wrangler.toml configuration file from local workspace
